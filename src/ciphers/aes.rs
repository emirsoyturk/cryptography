...
impl AdvancedEncryptionStandard for AdvancedEncryptionStandard128Bit {
    ...
    fn mix_columns_inversed(&mut self, state: &mut [[u8; 4]; 4]) {
        ...
        for c in 0..4 {
            temp[c][0] = galois_multiplication(0x0e, state[c][0]) ^ galois_multiplication(0x0b, state[c][1]) ^ galois_multiplication(0x0d, state[c][2]) ^ galois_multiplication(0x09, state[c][3]);
            temp[c][1] = galois_multiplication(0x09, state[c][0]) ^ galois_multiplication(0x0e, state[c][1]) ^ galois_multiplication(0x0b, state[c][2]) ^ galois_multiplication(0x0d, state[c][3]);
            temp[c][2] = galois_multiplication(0x0d, state[c][0]) ^ galois_multiplication(0x09, state[c][1]) ^ galois_multiplication(0x0e, state[c][2]) ^ galois_multiplication(0x0b, state[c][3]);
            temp[c][3] = galois_multiplication(0x0b, state[c][0]) ^ galois_multiplication(0x0d, state[c][1]) ^ galois_multiplication(0x09, state[c][2]) ^ galois_multiplication(0x0e, state[c][3]);
        }
...
impl Cipher for AdvancedEncryptionStandard128Bit {
    fn encrypt(&mut self, input: &[u8]) -> Vec<u8>{
        ...
        AdvancedEncryptionStandard::add_round_key(self, &mut state, &mut round, false);
    ...
    fn decrypt(&mut self, input: &[u8]) -> Vec<u8>{
        ...
        AdvancedEncryptionStandard::add_round_key(self, &mut state, &mut round, true);

        for _ in 0..9 {
            AdvancedEncryptionStandard::shift_rows_inversed(self, &mut state);
            AdvancedEncryptionStandard::sub_bytes_inversed(self, &mut state);
            AdvancedEncryptionStandard::add_round_key(self, &mut state, &mut round, true);
            AdvancedEncryptionStandard::mix_columns_inversed(self, &mut state);
        }
...
mod tests {
    ...
    fn test_round_increase_by_add_round_key() {
        ...
        for _ in 0..3 {
            AdvancedEncryptionStandard::add_round_key(&mut aes, &mut state, &mut round, false);
        }
    ...
    fn test_decrypt() {
        ...
        let mut aes = AdvancedEncryptionStandard128Bit { round_keys };
        let input:[u8; 16] = [0xc7, 0xd1, 0x24, 0x19, 0x48, 0x9e, 0x3b, 0x62, 0x33, 0xa2, 0xc5, 0xa7, 0xf4, 0x56, 0x31, 0x72];

        let output = Cipher::decrypt(&mut aes, &input);

        let expected_output = [0x00, 0x00, 0x01, 0x01, 0x03, 0x03, 0x07, 0x07, 0x0f, 0x0f, 0x1f, 0x1f, 0x3f, 0x3f, 0x7f, 0x7f];
